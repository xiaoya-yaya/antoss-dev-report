...
2. GitHub Events API 一直存在随机概率的数据缺失，但比例较小。在今年，GitHub 对 API 进行了[升级](https://github.blog/changelog/2025-08-08-upcoming-changes-to-github-events-api-payloads/)，减少了 payload 中的一部分字段信息。升级后，数据开始出现严重的缺失：10 月份有一周左右的日志数据全部丢失（Oct 08-14），且 10/08 以来的所有 PR 合入事件也都丢失。也就是说，通过 Events API，10/08 以来全域查不到一个 PR 的合入。这一数据故障影响了很多依赖于 Events API 的下游应用，例如，[OSSInsight](https://ossinsight.io) 洞察看板显示，VSCode 项目近 28 天内没有一个 PR 合入，而这显然和事实不符。
